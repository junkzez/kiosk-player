...
* Video kiosk for low-end Android TV boxes.
        * No AndroidX / no ExoPlayer on purpose: plain MediaPlayer + TextureView,
* hardware decoding, ~60 KB APK, works fine with 1 GB RAM.
        * Uses TextureView instead of SurfaceView so playback also renders correctly
        * on newer/other-vendor Android TV boxes where SurfaceView overlays can stay blank.
        */
...
@Override
                public void onSurfaceTextureAvailable(SurfaceTexture st, int width, int height) {
                        playbackSurface = new Surface(st);
                        surfaceReady = true;
                        if (mp == null) scanAndPlay(); else mp.setSurface(playbackSurface);
                }
...
@Override
                public boolean onSurfaceTextureDestroyed(SurfaceTexture st) {
                        surfaceReady = false;
                        if (mp != null) { try { mp.setSurface(null); } catch (Throwable ignored) {} }
                        if (playbackSurface != null) { playbackSurface.release(); playbackSurface = null; }
                        return true;
                }
